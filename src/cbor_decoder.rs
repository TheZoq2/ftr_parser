...
impl <R: Read + Seek>CborDecoder<R>{
    ...
    pub fn read_tag(&mut self) -> eyre::Result<i64> {
        let length = Self::read_major_type(self, TYPE_TAG)?;
        Self::read_unsigned_int(self, length , false)
    }


    pub fn read_major_type(&mut self, major_type: u8) -> eyre::Result<u8> {
    ...
    pub fn read_major_type_with_size(&mut self, major_type: u8) -> eyre::Result<i64> {
        let length = Self::read_major_type(self, major_type)?;
        Self::read_unsigned_int(self, length, true)
    }

    pub fn read_major_type_exact(&mut self, major_type: u8, sub_type: u8) -> eyre::Result<()> {
    ...
    pub fn read_array_length(&mut self) -> eyre::Result<i64> {
        Self::read_major_type_with_size(self, TYPE_ARRAY)
    }

    pub fn read_unsigned_int(&mut self, length: u8, break_allowed: bool) -> eyre::Result<i64> {
        let mut result = -1;
        if length < 24 {
    ...
    fn read_unsigned_int_8(&mut self) -> eyre::Result<i64> {
        let mut buf = vec![0u8; 1];
        self.input_stream.read_exact(&mut buf)?;
        Ok(buf[0] as i64)
    }

    fn read_unsigned_int_16(&mut self) -> eyre::Result<i64> {
        let mut buf = vec![0u8; 2];
        self.input_stream.read_exact(&mut buf)?;
        Ok((buf[0] as i64) << 8 | (buf[1] as i64))
    }

    fn read_unsigned_int_32(&mut self) -> eyre::Result<i64> {
        let mut buf = vec![0u8; 4];
        self.input_stream.read_exact(&mut buf)?;
        Ok((buf[0] as i64) << 24 | (buf[1] as i64) << 16 | (buf[2] as i64) << 8 | (buf[3] as i64))
    }

    fn read_unsigned_int_64(&mut self) -> eyre::Result<i64> {
    ...
    pub fn read_boolean(&mut self) -> eyre::Result<bool> {
        let b = self.read_major_type(TYPE_FLOAT_SIMPLE)?;
        Ok(b == TRUE)
    }

    #[allow(dead_code)]
    pub fn read_double(&mut self) -> eyre::Result<f64> {
    ...
    pub fn read_float(&mut self) -> eyre::Result<f32> {
    ...
    #[allow(dead_code)]
    pub fn read_half_precision_float(&mut self) -> eyre::Result<f16> {
    ...
    pub fn read_byte_string(&mut self) -> eyre::Result<Vec<u8>> {
    ...
    pub fn skip_byte_string(&mut self) -> eyre::Result<()> {
        let len = Self::read_major_type_with_size(self, TYPE_BYTE_STRING)?;
    ...
    pub fn read_int(&mut self) -> eyre::Result<i64> {
        let mut buf = vec![0u8; 1];
        self.input_stream.read_exact(&mut buf)?;
    ...
    pub fn expect_integer_type(&mut self, ib: u8) -> eyre::Result<i64> {
        let major_type = (ib & 0xff) >> 5;
        if (major_type != TYPE_UNSIGNED_INT) && (major_type != TYPE_NEGATIVE_INT) {
            Err(eyre::Report::msg("Expected Integer type!"))
        } else {
            Ok(-(major_type as i64))
        }
    }

    pub fn read_map_length(&mut self) -> eyre::Result<i64> {
        Self::read_major_type_with_size(self, TYPE_MAP)
    }

    pub fn read_text_string(&mut self) -> eyre::Result<String> {
    ...
    pub fn peek(&mut self)  -> eyre::Result<i64> {
